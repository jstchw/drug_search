...
const isValidSearchType = (type: string | null): boolean =>
  !!type && searchTypes.some((t) => t.param === type);
const isValidSex = (sex: string | null): boolean =>
  !!sex && searchSex.some((s) => s.param === sex);
const isValidAge = (age: string | null): boolean =>
  age !== null && !isNaN(Number(age)) && Number(age) >= 0;
...
const validateParams = (params: URLSearchParams): ValidationResult => {
  return {
    ...
    country: params.has("country")
      ? isValidCountry(params.get("country"))
      : null,
  };
};
...
const parseUrlParams = (
  ...
  const areParamsValid = Object.values(validation).every(
    (val) => val !== false,
  );
  if (!areParamsValid) {
    setError(true);
...
export const useUrlParams = () => {
  ...
  const params = useMemo(
    () => parseUrlParams(location.search, setError),
    [location.search],
  );
