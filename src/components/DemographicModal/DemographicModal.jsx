...
const prepareDataForGPT = (data) => {
  ...
  for (let key in gptData) {
    if (gptData.hasOwnProperty(key)) {
      ...
      let sortedTerms = Object.entries(gptData[key].termCountDict).sort(
        (a, b) => b[1] - a[1],
      );
      gptData[key].termCountDict = sortedTerms
        .slice(0, maxTerms)
      ...
      <Popover>
        <Popover.Body>
          ...
          <div className={"fs-6 text-center"}>
            <strong>{count.toLocaleString()}</strong> cases recorded
          </div>
        </Popover.Body>
      </Popover>
    );
  ...
  const showAIWarning = () => {
    const popover = (
      <Popover>
        <Popover.Body>
          <div className={"fs-6 text-center"}>
            This content is generated using GPT 3.5 model and may contain false
            information. Always cross-verify with reliable sources and consult
              ...
              <Wrapper key={index}>
                {chunk.map(([key, value], subIndex) => (
                  <Col className={"text-center"} key={subIndex}>
                    <h4>
                      {getDemographicDef(key, value.totalCount, value.def)}
                    </h4>
                    <ApexChart
...
const DemographicModal = ({ show, handleClose, selectedWord, isMobile }) => {
  ...
  return (
    <Modal centered show={show} onHide={handleOnHide} size={"xl"}>
      ...
      <Modal.Body>
        {shouldRenderContent && word && (
          <DemographicModalInfo word={word} isMobile={isMobile} />
        )}
      </Modal.Body>
    </Modal>
  );
