    ...
    return (
      <>
        <div>
          <ListGroup>
            <ListGroup.Item>
              <div className={'fs-5'} style={{ width: 'fit-content' }}>
                No search history
              </div>
            </ListGroup.Item>
          </ListGroup>
              ...
              <Popover>
                ...
                <Popover.Body>
                  <div className={'d-flex align-items-center fs-5'}>
                    ...
                    <span>
                      {item.sex ? (
                        mapParamToLabel(item.sex, searchSex)
                      ) : (
                  ...
                  <div className={'d-flex align-items-center fs-5'}>
                    ...
                    <div className={'vr mx-2'} />
                    <span>
                      {item.age && item.age.min ? item.age.min : <span className={'text-secondary'}>N/a</span>}
                    </span>
                    <span> - </span>
                    <span>
                      {item.age && item.age.max ? item.age.max : <span className={'text-secondary'}>N/a</span>}
                    </span>
                  </div>
                  <div className={'d-flex align-items-center fs-5'}>
                    <Globe weight={'light'} />
            ...
            return (
              <ListGroup.Item key={index}>
                <OverlayTrigger overlay={popover} key={index} trigger={['hover', 'focus']} placement="bottom">
                  <div
                    onClick={() => handleRedirect(item.terms, item)}
                    className={'fs-5'}
                    key={index}
